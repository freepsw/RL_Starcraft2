...
# Attack Point를 4개로 단순화 (64 mod 32 (X, Y 2조합으로)
...
class QLearningTable:
    ...
    def choose_action(self, observation, excluded_actions=[]):
        ...
        print("disallow: ", self.disallowed_actions[observation])
        print("exclude : ", excluded_actions)

        #state_action = self.q_table.ix[observation, :]
        #state_action = self.q_table.loc[observation, self.q_table.columns[:]]
        state_action = self.q_table.loc[observation, :]
        print("before state_action: ", state_action)
        for excluded_action in excluded_actions:
            del state_action[excluded_action]

        print("after state_action: ", state_action)
    ...
    # Terminal State(게임이 끝났을때)에는 reward를 제공해 주는 그대로 적용한다. (Q-Value를 적용하지 않음)
    def learn(self, s, a, r, s_):
        # 기존 상태와 동일한 상태가 들어오면, 학습하지 않는다. (무시)
        if s == s_:
            return
        ...
        # Terminal State를 구분하여 Q-Value 적용
        if s_ != 'terminal':
...
class TerranSparseRewardRLAgent(base_agent.BaseAgent):
    ...
    def step(self, obs):
        ...
        if self.move_number == 0:
            ...
            #     ACTION_BUILD_MARINE, #3
            # 4번 부터는 attack관련 action
            # ]
            # 특정 조건에서 특정 action이 선택되지 않도록 설정 (이런 설정이 필요한가?)
            # https://itnext.io/refine-your-sparse-pysc2-agent-a3feb189bc68
            # 위 설명을 보면, 초반 마린 러시를 위한 가이드를 제공하는 듯
            excluded_actions = []
            # 배럭이 2개 or SCV가 없으면 supply depot를 짓지 말자
            if supply_depot_count == 2 or worker_supply == 0:
                excluded_actions.append(1)

            # 이미 배럭이 2개거나 서플라이디포 또는 scv가 없으면 배럭을 짓지 말자.
            if supply_depot_count == 0 or barracks_count == 2 or worker_supply == 0:
                excluded_actions.append(2)

            # 현재 기준 인구수가 찾거나 배럭이 없으면, 마린을 생성하지 말자
